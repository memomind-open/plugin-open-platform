...
def ensure_toolchain(insecure: bool = False) -> pathlib.Path:
    ...
    if override:
        ...
        # Windows network drives often resolve to a long UNC path. The xPack
        # archive contains deep C++ header paths that exceed legacy MAX_PATH,
        # so use a short per-user local cache on Windows.
        if system_arch[0] == "windows":
            local = os.environ.get("LOCALAPPDATA")
            cache = pathlib.Path(local) / "GMPluginSDK/toolchains" if local else pathlib.Path.home() / ".gmpluginsdk/toolchains"
        else:
            cache = SDK / ".toolchains"
...
def cmake_build_path(build_root: pathlib.Path) -> pathlib.Path:
    ...
    return build_root / ".cmake" / f"{system}-{architecture}"
...
def configure_cmake(
    ...
    cmake_build = cmake_build_path(build_root)
    command = [
        cmake, "-S", SDK, "-B", cmake_build, "-G", "Ninja",
...
def build_cmake(
    ...
    command: list[object] = [cmake, "--build", cmake_build_path(build_root), "--parallel"]
...
def main() -> int:
    ...
    parser.add_argument("--build-dir", type=pathlib.Path, default=SDK / "build-host")
    ...
    if args.command == "clean":
        filesystem_root = pathlib.Path(build_root.anchor)
        try:
            relative_to_sdk = build_root.relative_to(SDK)
            in_sdk_build_tree = (bool(relative_to_sdk.parts) and
                                 relative_to_sdk.parts[0].startswith("build"))
        except ValueError:
            in_sdk_build_tree = False
        if ((not build_root.name.startswith("build") and
             not in_sdk_build_tree) or
                build_root == filesystem_root or
                build_root.parent == filesystem_root or
                build_root == pathlib.Path.home().resolve() or
                build_root == SDK or build_root in SDK.parents):
            raise RuntimeError(f"refusing to clean unsafe build directory: {build_root}")
        shutil.rmtree(build_root, ignore_errors=True)
        shutil.rmtree(cmake_build_path(build_root), ignore_errors=True)
    ...
    if args.command == "inspect":
        run([tool(bin_dir, "readelf"), "-h", "-l", "-S", "-r", "-s", gmp.with_suffix(".elf")])
    print(f"Built {gmp}")
    return 0
...
def cli() -> int:
    try:
        ...
        print(f"gm_build: {error}", file=sys.stderr)
        return 1
