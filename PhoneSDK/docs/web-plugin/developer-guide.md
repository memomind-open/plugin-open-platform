...
`gm-phone-plugin-devkit-0.1.0.zip`. The ZIP contains the browser SDK, PhoneSDK
...
- `studio/`: PhoneSDK Browser Studio for local App Bridge, display, event, and
...
`Studio`, `PhoneSDK`, and `GlassSDK` directories together so it can discover the
public examples automatically. Discovery is recursive below `PhoneSDK/examples`
and does not require a predefined plugin directory name.

Use PhoneSDK Browser Studio for lightweight Web-only checks that do not require a
running `.gmp`.
...
Browser Studio, packager, examples, and documentation. If PhoneSDK libraries are
