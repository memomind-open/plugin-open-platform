...
`gm-phone-plugin-devkit-<version>.zip`. The DevKit ZIP includes the browser SDK,
PhoneSDK Browser Studio, packager, examples, and documentation, so it does not
...
```sh
mkdir -p ./vendor
cp /path/to/gm-phone-plugin-devkit/sdk/gm-plugin-web-sdk.esm.js ./vendor/
```

When working from the full PhoneSDK source tree, run `npm run build:devkit` first
...
Keep `Studio`, `PhoneSDK`, and `GlassSDK` together in the repository layout so
Desktop Studio can discover the public examples automatically.

### Lightweight option: PhoneSDK Browser Studio

For a static H5 project with `index.html` at its root:

```sh
node /path/to/PhoneSDK/tools/run-browser-studio.mjs \
...
```sh
node /path/to/gm-phone-plugin-devkit/studio/gm-plugin-studio.mjs \
  --plugin /absolute/path/to/my-plugin
```
...
## Future PhoneSDK package workflow

If the PhoneSDK libraries are published to npm in the future, plugin projects
...
remains part of the public PhoneSDK and DevKit ZIP; no public Desktop Studio
