#!/usr/bin/env python3
"""Discover and incrementally package every PhoneSDK Web plugin example."""
...
DIST = ROOT / "dist"
PACKAGER = ROOT / "tools" / "build-mmpkg.mjs"
...
class WebPlugin:
    ...
    def output(self) -> pathlib.Path:
        ...
        name = "-".join(self.workspace.relative_to(EXAMPLES).parts)
        return DIST / f"{name}-{version}.mmpkg"
...
def fingerprint(roots: list[pathlib.Path], ignored: set[str] = ALWAYS_IGNORED) -> str:
    ...
    for root in sorted(roots, key=lambda item: item.as_posix()):
        files = [root] if root.is_file() else list(iter_files(root, ignored))
        for path in files:
...
def nearest_workspace(manifest: pathlib.Path) -> tuple[pathlib.Path, dict[str, object] | None]:
...
def discover_plugins() -> list[WebPlugin]:
    ...
    for manifest_path in sorted(EXAMPLES.rglob("manifest.json")):
        ...
        workspace, package_json = nearest_workspace(manifest_path)
        ...
        plugins.append(WebPlugin(
            key=f"plugin:{workspace.relative_to(ROOT).as_posix()}",
            workspace=workspace,
...
def build_plugin(plugin: WebPlugin, state: dict[str, object]) -> None:
    if plugin.has_build_script:
        ...
        if (not (plugin.workspace / "node_modules").is_dir()
            ...
            # Dependency installation is a build prerequisite. Disable the
            # unrelated registry audit/funding requests so cached installs do
            # not hang in restricted or proxied build environments.
            run([
...
def build_once(force: bool = False, quiet: bool = False) -> bool:
    state = load_state()
    ...
    for plugin in discover_plugins():
        ...
        ignored = (
            ALWAYS_IGNORED | GENERATED_DIRECTORIES
            if plugin.has_build_script else ALWAYS_IGNORED
        )
        ...
        if force or targets.get(plugin.key) != current or not plugin.output.is_file():
            print(f"PhoneSDK: building {plugin.workspace.relative_to(ROOT)}")
            build_plugin(plugin, state)
            targets[plugin.key] = current
            built_anything = True
    for key in list(targets):
        if key.startswith("plugin:") and key not in discovered_keys:
            del targets[key]
    save_state(state)
    if not built_anything and not quiet:
        print("All PhoneSDK plugins are up to date")
    return built_anything


def list_plugins() -> None:
    print("PhoneSDK plugins:")
    for plugin in discover_plugins():
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Discover and incrementally build every PhoneSDK Web plugin.",
        epilog="""Build examples:
  Ubuntu/macOS:
    ./build.py              Build only changed Web plugins
    ./build.py --force      Rebuild every Web plugin
    ./build.py --watch      Watch for changes and rebuild continuously
    ./build.py --list       Show discovered inputs and output files

  Windows PowerShell:
    py build.py             Build only changed Web plugins
    py build.py --force     Rebuild every Web plugin
    py build.py --watch     Watch for changes and rebuild continuously
    py build.py --list      Show discovered inputs and output files

Output:
  dist/<plugin-name>-<version>.mmpkg

The first build of a plugin with package.json may run npm ci or npm install.
Incremental state is stored in .build/build-state.json.""",
    ...
    parser.add_argument("--force", action="store_true", help="rebuild every plugin")
    parser.add_argument("--watch", action="store_true", help="keep scanning and rebuild changes")
    parser.add_argument("--list", action="store_true", help="list plugins without building")
    ...
    if args.list:
        ...
        return 0
    if not args.watch:
        build_once(args.force, args.quiet)
        return 0
    print("Watching for PhoneSDK plugin changes; press Ctrl+C to stop")
    first = True
    try:
        while True:
            build_once(args.force and first, quiet=not first)
            first = False
            time.sleep(1)
