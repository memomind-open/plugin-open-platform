...
(for example, `PhoneSDK/examples/<plugin>/manifest.json`). SDK infrastructure
...
`PhoneSDK/examples` and `GlassSDK/examples` remain the recommended locations, but
...
The separate PhoneSDK Browser Studio is public source under
`PhoneSDK/tools/browser-studio` and is also included in the PhoneSDK DevKit ZIP. It
...
[Web Plugin Studio guide](../PhoneSDK/docs/web-plugin/studio.md) for a detailed
comparison.
