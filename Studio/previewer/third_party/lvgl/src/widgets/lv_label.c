...
void lv_label_set_text(lv_obj_t * obj, const char * text)
{
    ...
    /* 通用短路：上层(导航/字幕/会议等)常高频重设"相同文本"，而本函数默认每次都
     * lv_obj_invalidate 触发整块重绘。文本内容未变时直接返回，让所有 app 无需各自做
     * 变化检测即可避免无谓重绘。仅处理最常见且语义安全的场景：传入非空 + 当前为动态文本
     * + 非自赋值 + 内容完全相同；传 NULL(自刷新)、static 文本、自赋值仍走原逻辑。
     * 文本未变时也不清 size_cache_valid，省去尺寸重算。 */
    if (text != NULL && label->text != NULL && label->static_txt == 0
        && text != label->text && strcmp(label->text, text) == 0) {
        return;
    }
#endif
...
static void lv_label_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    /* 初始化缓存字段 */
    label->last_max_w = -1;
    label->cached_size.x = 0;
    label->cached_size.y = 0;
    label->cached_font = NULL;
    label->size_cache_valid = 0;
#endif
...
#if defined(CONFIG_XGIMI_PATCH)
        /* 样式(字体/行距/字距等)变化会影响文本尺寸，缓存 key 无法覆盖全部样式项，
         * 这里直接失效尺寸缓存，避免行距/字距变化后命中过期缓存导致裁剪/高亮错位。 */
        ((lv_label_t *)obj)->size_cache_valid = 0;
#endif
...
#if defined(CONFIG_XGIMI_PATCH)
        /* 性能优化：使用缓存(key 含 max_w/字体/行距/字距，任一变化都重算) */
        if(label->size_cache_valid && w == label->last_max_w && font == label->cached_font
           && line_space == label->cached_line_space && letter_space == label->cached_letter_space) {
            size = label->cached_size;
        } else {
            lv_txt_get_size(&size, label->text, font, letter_space, line_space, w, flag);
            /* 更新缓存 */
            label->last_max_w = w;
            label->cached_size = size;
            label->cached_font = font;
            label->cached_line_space = line_space;
            label->cached_letter_space = letter_space;
            label->size_cache_valid = 1;
        }
#else
        lv_txt_get_size(&size, label->text, font, letter_space, line_space, w, flag);
#endif
...
#if defined(CONFIG_XGIMI_PATCH)
        /* 选区背景不透明度从样式取，透明则跳过逐字符背景矩形(省无效填充)。 */
        label_draw_dsc.sel_bg_opa = lv_obj_get_style_bg_opa(obj, LV_PART_SELECTED);
#endif
    }
...
#if defined(CONFIG_XGIMI_PATCH)
    /* 性能优化：如果 max_w <= 0，跳过文本计算
     * 因为布局还没确定，后续 SIZE_CHANGED 事件会再次触发计算 */
    // if(max_w <= 0) {
    //     LV_LOG_INFO("lv_label_refr_text: max_w=%d, skip", max_w);
    //     label->last_max_w = -1; /* 标记为无效，等布局后再计算 */
    //     label->size_cache_valid = 0;
    //     return;
    // }
    lv_coord_t w_style = lv_obj_get_style_width(obj, LV_PART_MAIN);
#endif
...
#if defined(CONFIG_XGIMI_PATCH)
    if(w_style == LV_SIZE_CONTENT && !obj->w_layout) flag |= LV_TEXT_FLAG_FIT;


    /* 性能优化：缓存有效且 max_w/字体/行距/字距都相同时，使用缓存值 */
    if(label->size_cache_valid && max_w == label->last_max_w && font == label->cached_font
       && line_space == label->cached_line_space && letter_space == label->cached_letter_space) {
        size = label->cached_size;
        LV_LOG_INFO("lv_label_refr_text: use cached size, max_w=%d", max_w);
    } else {
        uint32_t t_get_size_start = lv_tick_get();
        lv_txt_get_size(&size, label->text, font, letter_space, line_space, max_w, flag);
        uint32_t t_get_size_cost = lv_tick_get() - t_get_size_start;

        /* 更新缓存 */
        label->last_max_w = max_w;
        label->cached_size = size;
        label->cached_font = font;
        label->cached_line_space = line_space;
        label->cached_letter_space = letter_space;
        ...
        /* 打印耗时超过 5ms 的 label 文本计算 */
        if(t_get_size_cost >= 5) {
            LV_LOG_WARN("lv_label_refr_text: txt_get_size=%u ms, max_w=%d, long_mode=%d",
                       t_get_size_cost, max_w, label->long_mode);
        }
    }
#else
    if(lv_obj_get_style_width(obj, LV_PART_MAIN) == LV_SIZE_CONTENT && !obj->w_layout) flag |= LV_TEXT_FLAG_FIT;
