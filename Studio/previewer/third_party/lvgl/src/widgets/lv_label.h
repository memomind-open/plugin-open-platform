...
typedef struct {
    ...
    /*  性能优化：缓存文本尺寸计算结果 */
    lv_coord_t last_max_w;              /* 上次计算时的最大宽度 */
    lv_point_t cached_size;             /* 缓存的文本尺寸 */
    const void * cached_font;           /* 上次计算时的字体指针 */
    lv_coord_t cached_line_space;       /* 上次计算时的行距 */
    lv_coord_t cached_letter_space;     /* 上次计算时的字距 */
    uint8_t size_cache_valid : 1;       /* 缓存是否有效 */
#endif	
} lv_label_t;
