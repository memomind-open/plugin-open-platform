...
void lv_canvas_fill_bg(lv_obj_t * canvas, lv_color_t color, lv_opa_t opa)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_INDEXED_1BIT) {
        ...
        uint32_t x;
        uint32_t y;
        if(!color.full && (dsc->header.cf == LV_IMG_CF_TRUE_COLOR || dsc->header.cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED)) {
            uint8_t px_size = lv_img_cf_get_px_size(dsc->header.cf) >> 3;
            uint32_t len_wr = dsc->header.w * dsc->header.h  * px_size;
            lv_memset((uint8_t *)dsc->data, 0x00, len_wr);
        }else{   
