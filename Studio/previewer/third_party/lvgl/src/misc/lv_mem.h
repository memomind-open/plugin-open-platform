...
static inline void * LV_ATTRIBUTE_FAST_MEM lv_memcpy_small(void * dst, const void * src, size_t len)
{
    ...
    // Unroll loop for small sizes (1-8 bytes) for better performance
    switch(len) {
        case 8: *d8++ = *s8++; // fallthrough
        case 7: *d8++ = *s8++; // fallthrough
        case 6: *d8++ = *s8++; // fallthrough
        case 5: *d8++ = *s8++; // fallthrough
        case 4: *d8++ = *s8++; // fallthrough
        case 3: *d8++ = *s8++; // fallthrough
        case 2: *d8++ = *s8++; // fallthrough
        case 1: *d8++ = *s8++; // fallthrough
        case 0: break;
        default:
            // For larger sizes, use the original loop
            while(len--) {
                *d8++ = *s8++;
            }
            break;
