...
#if defined(CONFIG_XGIMI_PATCH)
#define TXT_SIZE_CACHE_SIZE 32
#define GLYPH_WIDTH_CACHE_SIZE 192
#endif

static bool lv_txt_is_join_punctuation(uint32_t letter)
{
    switch(letter) {
        case 0x002D: /* - */
        case 0x3002: /* 。 */
...
/* Closing CJK punctuation is a soft line boundary: it stays with the text
 * before it, while the following word can still be placed on the same line. */
static bool lv_txt_is_cjk_break_after_punctuation(uint32_t letter)
{
    switch(letter) {
        case 0x3002: /* 。 */
        case 0x3001: /* 、 */
...
typedef struct {
    ...
    lv_coord_t width;
    bool valid;
} lv_glyph_width_cache_t;

/********************** *  STATIC VARIABLES **********************/
static lv_txt_size_cache_t txt_size_cache[TXT_SIZE_CACHE_SIZE];
static uint8_t txt_size_cache_idx = 0;
static lv_glyph_width_cache_t glyph_width_cache[GLYPH_WIDTH_CACHE_SIZE];
static uint8_t glyph_width_cache_idx = 0;
#endif

#if defined(CONFIG_XGIMI_PATCH)
static lv_coord_t lv_txt_get_cached_glyph_width(const lv_font_t *font, uint32_t letter, uint32_t letter_next)
{
    if(font == NULL) {
        return 0;
    }

    for(uint8_t i = 0; i < GLYPH_WIDTH_CACHE_SIZE; i++) {
        lv_glyph_width_cache_t *entry = &glyph_width_cache[i];
        if(entry->valid && entry->font == font && entry->letter == letter) {
            return entry->width;
        }
    }

    lv_coord_t width = lv_font_get_glyph_width(font, letter, letter_next);

    lv_glyph_width_cache_t *slot = &glyph_width_cache[glyph_width_cache_idx++];
    if(glyph_width_cache_idx >= GLYPH_WIDTH_CACHE_SIZE) {
        glyph_width_cache_idx = 0;
    }
    slot->font = font;
    slot->letter = letter;
    slot->width = width;
                     ...
                     lv_coord_t line_space, lv_coord_t max_width, lv_text_flag_t flag)
{
    ...
    /* 检查缓存 */
    if(text[0] != '\0') {
        for(uint8_t i = 0; i < TXT_SIZE_CACHE_SIZE; i++) {
            lv_txt_size_cache_t *cache = &txt_size_cache[i];
            if(cache->text && cache->font == font && cache->max_w == max_width &&
               cache->letter_space == letter_space && cache->line_space == line_space &&
               cache->flag == flag && strcmp(cache->text, text) == 0) {
                *size_res = cache->size;
                LV_LOG_INFO("lv_txt_get_size: use cached size, max_w=%d,i=%d", max_width, i);
                return;
            }
        }
    }
    ...
    /* 保存到缓存 */
    if(text[0] != '\0') {
        lv_txt_size_cache_t *cache = &txt_size_cache[txt_size_cache_idx++];
        if(txt_size_cache_idx >= TXT_SIZE_CACHE_SIZE) txt_size_cache_idx = 0;
        if(cache->text) lv_mem_free(cache->text);
        cache->text = lv_mem_alloc(strlen(text) + 1);
        if(cache->text) {
            strcpy(cache->text, text);
        }
        cache->font = font;
        cache->max_w = max_width;
        cache->letter_space = letter_space;
                                     ...
                                     lv_text_flag_t flag, uint32_t * word_w_ptr, lv_text_cmd_state_t * cmd_state, bool force)
{
    ...
    while(txt[i] != '\0') {
        ...
        letter_w = lv_txt_get_cached_glyph_width(font, letter, letter_next);
        cur_w += letter_w;
        ...
        if(letter == '\n' || letter == '\r' ||
           (_lv_txt_is_break_char(letter) && !lv_txt_is_cjk_break_after_punctuation(letter))) {
        ...
        if(lv_txt_is_cjk_break_after_punctuation(letter)) {
            if(break_index == NO_BREAK_FOUND) {
                if(word_w_ptr != NULL) *word_w_ptr = cur_w;
                i = i_next;

                /* Keep a following recolor closing marker on this line. */
                if((flag & LV_TEXT_FLAG_RECOLOR) != 0 &&
                   *cmd_state == LV_TEXT_CMD_STATE_IN &&
                   txt[i] == LV_TXT_COLOR_CMD[0]) {
                    uint32_t command_length = 0;
                    uint32_t command = _lv_txt_encoded_next(&txt[i], &command_length);
                    if(_lv_txt_is_cmd(cmd_state, command)) i += command_length;
                }
            }
            break;
        }
                               ...
                               lv_coord_t * used_width, lv_text_flag_t flag)
{
    ...
    while(txt[i] != '\0' && max_width > 0) {
        ...
        if(txt[i] == '\n' || txt[i] == '\r') {
            if(txt[i] == '\r' && txt[i + 1] == '\n') i += 2;
            else i++;  /*Include the following newline in the current line*/
            break;
        }
    ...
    if(i == 0) {
        ...
        if(used_width != NULL) {
            line_w = lv_txt_get_cached_glyph_width(font, letter, '\0');
        }
    }
    ...
    if(txt[i] != '\0') {
        /* 如果下一行开头是标点，尝试把行尾回退到合适位置，避免标点单独换行 */
        uint32_t next_ofs = 0;
        uint32_t next_letter = _lv_txt_encoded_next(txt + i, &next_ofs);
        bool join_punct = lv_txt_is_join_punctuation(next_letter);
        TXBUG("txt: %s, join punctuation: %d, letter: %d", txt + i, join_punct, next_letter);

        if(join_punct && i > 0) {
            if (txt[i - 1] == '\n' || txt[i - 1] == '\r') {
                TXBUG("previous line ends with newline, no adjust");
                return i;
            }
            uint32_t adjusted_i = i;

            while(adjusted_i > 0) {
                /* 逐个回退 UTF-8 字符，确认上一字符是否也是需要连排的标点 */
                uint32_t prev_pos = adjusted_i;
                uint32_t prev_letter = _lv_txt_encoded_prev(txt, &prev_pos);

                if(prev_pos == adjusted_i || prev_letter == 0) {
                    break;
                }

                uint32_t prev_char_bytes = adjusted_i - prev_pos;
                if(prev_char_bytes == 0) {
                    break;
                }

                adjusted_i = prev_pos;

                TXBUG("prev letter:%u, prev_char_bytes:%u", prev_letter, prev_char_bytes);

                uint32_t lookahead_ofs = 0;
                uint32_t lookahead_letter = _lv_txt_encoded_next(txt + adjusted_i, &lookahead_ofs);

                if(!lv_txt_is_join_punctuation(lookahead_letter)) {
                    break;
                }
            }

            if(adjusted_i != i && adjusted_i > 0) {
                /* 回退成功后重算行宽，保持与新断点一致 */
                if(used_width != NULL) {
                    line_w = lv_txt_get_width(txt, adjusted_i, font, letter_space, flag);
                    *used_width = line_w;
                }
                TXBUG("adjusted next line index: %d, txt:%s", adjusted_i, txt);
                return adjusted_i;
            }
        }
    }
                            ...
                            lv_text_flag_t flag)
{
    ...
    if(length != 0) {
        while(i < length) {
            ...
            lv_coord_t char_width = lv_txt_get_cached_glyph_width(font, letter, letter_next);
            if(char_width > 0) {
                width += char_width;
                width += letter_space;
            }
