...
#if GM_SUPPORT_COLOR_RBG224 //xgimi rbg224

#if GM_SUPPORT_G4_FAST
# define LV_COLOR_SET_R8(c, v)
# define LV_COLOR_SET_G8(c, v) (c).ch.green = (uint8_t)((v) & 0xfU)
# define LV_COLOR_SET_B8(c, v)
# define LV_COLOR_SET_A8(c, v)
#else
# define LV_COLOR_SET_R8(c, v) (c).ch.red = (uint8_t)((v) & 0x3U)
# define LV_COLOR_SET_G8(c, v) (c).ch.green = (uint8_t)((v) & 0xfU)
...
#if GM_SUPPORT_G4_FAST
# define LV_COLOR_GET_R8(c) 0
# define LV_COLOR_GET_B8(c) 0
#else
...
#if GM_SUPPORT_COLOR_RBG224 //xgimi rbg224
#if GM_SUPPORT_G4_FAST
# define LV_COLOR_MAKE8(r8, g8, b8) {{0, 0, (uint8_t)(g8 >> 4)}}
#else
# define LV_COLOR_MAKE8(r8, g8, b8) {{(uint8_t)((r8 >> 6) & 0x3U), (uint8_t)((b8 >> 6) & 0x3U),(uint8_t)((g8 >> 4) & 0xfU)}}
#endif
#else //rgb332
...
#elif LV_COLOR_DEPTH == 8
#if GM_SUPPORT_COLOR_RBG224 //xgimi rbg224
    if((LV_COLOR_GET_R(color) & 0x2) || (LV_COLOR_GET_G(color) & 0x8) || (LV_COLOR_GET_B(color) & 0x2)) {
        return 1;
    }
    else {
        return 0;
...
static inline uint16_t lv_color_to16(lv_color_t color)
{
    ...
    lv_color16_t ret;
#if GM_SUPPORT_COLOR_RBG224 //xgimi rbg224
    LV_COLOR_SET_R16(ret, LV_COLOR_GET_R(color) * 10);  /*(2^5 - 1)/(2^2 - 1) = 31/3 = 10*/
    LV_COLOR_SET_G16(ret, LV_COLOR_GET_G(color) * 4);  /*(2^6 - 1)/(2^4 - 1) = 63/15 = 4*/
...
static inline uint32_t lv_color_to32(lv_color_t color)
{
    ...
    lv_color32_t ret;
#if GM_SUPPORT_COLOR_RBG224 //xgimi rbg224
    LV_COLOR_SET_R32(ret, LV_COLOR_GET_R(color) * 85); /*(2^8 - 1)/(2^2 - 1) = 255/3 = 85*/
    LV_COLOR_SET_G32(ret, LV_COLOR_GET_G(color) * 17); /*(2^8 - 1)/(2^4 - 1) = 255/15 = 17*/
...
{
#if GM_SUPPORT_G4_FAST //xgimi rbg224
    UNUSED(r);
    UNUSED(b);
#endif
