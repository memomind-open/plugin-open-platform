...
void LV_ATTRIBUTE_FAST_MEM lv_draw_label_dsc_init(lv_draw_label_dsc_t * dsc)
{
    ...
    dsc->sel_bg_opa = LV_OPA_COVER; /* 默认保持原行为：画选区背景矩形 */
#endif
 ...
 */
void LV_ATTRIBUTE_FAST_MEM lv_draw_label(lv_draw_ctx_t * draw_ctx, const lv_draw_label_dsc_t * dsc,                                         const lv_area_t * coords, const char * txt, lv_draw_label_hint_t * hint)
{
...
#if defined(CONFIG_XGIMI_PATCH)
    /* 选区背景透明/极低不透明度(<=LV_OPA_MIN)时整体跳过逐字符 rect 填充(见 sel_bg_opa 说明)。 */
    draw_dsc_sel.bg_opa = dsc->sel_bg_opa;
    bool sel_draw_bg = (dsc->sel_bg_opa > LV_OPA_MIN);
#endif
                ...
                if(cmd_state == CMD_STATE_PAR) {
                    if(letter == ' ') {
                        ...
                        if(i - par_start == LABEL_RECOLOR_PAR_LENGTH + 1) {
                            ...
                            const char *pDa = &bidi_txt[par_start];
                            recolor = lv_color_make(0, hex_char_to_num(pDa[2]) << 4, 0);
